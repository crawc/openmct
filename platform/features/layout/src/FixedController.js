...
define(
    ...
    function (
        ...
        function FixedController($scope, $q, dialogService, openmct) {
            ...
            this.telemetryCollections = new Map();
            this.$scope = $scope;
            ...
            function updateDisplayBounds(bounds) {
                self.telemetryCollections.forEach(function (collection) {
        ...
        FixedController.prototype.unsubscribe = function () {
            ...
            this.telemetryCollections.forEach(function (collection) {
                collection.removeAllListeners();
                collection.clear();
            });
            this.telemetryCollections.clear();
            this.telemetryObjects = [];
        };
        ...
        FixedController.prototype.subscribeToObjects = function (objects) {
            ...
            this.subscriptions = objects.map(function (object) {
                return self.openmct.telemetry.subscribe(object, function callback(data) {
                    self.receiveTelemetry(object, [data]);
                }, {});
            });
            return objects;
        };
        ...
        /**
         * Returns a new or cached telemetry collection associated with a given
         * object.
         * @private
         * @param {object} object A domain object
         * @returns {TelemetryCollection} the collection of telemetry for the given object
         */
        FixedController.prototype.getCollectionForObject = function (object) {
            var telemetryCollection = this.telemetryCollections.get(object);
            var timeSystem;
            var domainKey;
            var metadata;
            var domainMetadata;
            ...
            if (telemetryCollection === undefined) {
                ...
                metadata = this.openmct.telemetry.getMetadata(object);
                domainMetadata = metadata.valuesForHints(['domain']).find(function (value) {
                    return value.key === timeSystem.metadata.key;
                });
                if (domainMetadata) {
                    domainKey = domainMetadata.source || domainMetadata.key;
                } else {
                    domainKey = timeSystem.metadata.key;
                }

                telemetryCollection = new TelemetryCollection();
                telemetryCollection.bounds(this.openmct.conductor.bounds());
                ...
                this.telemetryCollections.set(object, telemetryCollection);
            }
        ...
        FixedController.prototype.fetchHistoricalData = function (objects) {
            ...
            objects.forEach(function (object) {
                self.openmct.telemetry.request(object, {start: bounds.start, end: bounds.end, size: 1})
                    .then(self.receiveTelemetry.bind(self, object));
            });
            return objects;
        };
        ...
        /**
         * Process a telemetry datum received for an individual object
         * @private
         * @param {object} telemetryObject the domain object associated with the telemetry datum
         * @param {object} datum a telemetry datum
         */
        FixedController.prototype.receiveTelemetry = function (telemetryObject, datum) {
            this.getCollectionForObject(telemetryObject).add(datum);
        };
